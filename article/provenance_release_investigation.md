...
## Post-investigation status update

Route B was completed after this investigation. The article now uses the
documented `BH54S4ZB_20251117` BlueTopo tile, a grid-aligned 4 m crop, and
author-created analysis windows generated by
`article/data_provenance/acquire_bluetopo_example.R`. All real-data figures,
tables, sensitivity records, and clean-environment outputs were regenerated
from that Route-B material. The legacy `inst/extdata` fixtures remain excluded
from the article evidence.

The evaluated package was also released publicly as version 0.2.0 under
annotated tag and GitHub release `v0.2.0`. A persistent archive DOI has not
been assigned. The original pre-release findings below are retained as a dated
record of why Route B was selected; they are not a statement of the final
article data or release status.
